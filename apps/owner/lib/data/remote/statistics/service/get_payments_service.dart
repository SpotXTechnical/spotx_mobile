...
class GetPaymentsService extends BaseService {
  Future<ApiResponse> getPayments(int page, {StatisticsFilter? statisticsFilter}) async {
    ...
    NetworkRequest request =
        NetworkRequest(paymentApi, RequestMethod.get, headers: await getHeaders(), queryParams: queryParams);

    var result = await networkManager.perform(request);
    if (result.status == Status.OK) {
      result.data = BaseListResponse.fromJson(result.data, (json) => PaymentEntity.fromJson(json));
    }
    return result;
  }
}
