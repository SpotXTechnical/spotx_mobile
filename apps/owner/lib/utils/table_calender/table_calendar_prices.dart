...
class TableCalendarPrices<T> extends StatefulWidget {
  ...
  @override
  _TableCalendarPricesState<T> createState() => _TableCalendarPricesState<T>();
}

class _TableCalendarPricesState<T> extends State<TableCalendarPrices<T>> {
  ...
  @override
  void didUpdateWidget(TableCalendarPrices<T> oldWidget) {
        ...
        if (isReserved(day)) {
          rangeHighlight = Center(
            child: Container(
              ...
              height:
                  (shorterSide - widget.calendarStyle.cellMargin.vertical) * widget.calendarStyle.rangeHighlightScale,
              color: widget.calendarStyle.reservedHighlightColor ?? Theme.of(context).dividerColor,
            ),
          );
        }
  ...
  bool _isDayDisabled(DateTime day) {
    return day.isBefore(widget.firstDay) || day.isAfter(widget.lastDay) || !_isDayAvailable(day) || isReserved(day);
  }
  ...
  bool isReserved(DateTime day) {
    ...
    widget.priceRanges.asMap().forEach((key, value) {
      if (day.month >= value.startDay!.month && day.month <= value.endDay!.month) {
  ...
  bool isSelectedWithinReservationRange(DateTime start, DateTime end) {
    bool inRange = false;
    for (var i = 0; i < widget.priceRanges.length; i++) {
      inRange = isInRange(widget.priceRanges[i], start, end);
      if (inRange) break;
    }
    return inRange;
  }

  bool isInRange(PriceRange reservedRange, DateTime start, DateTime end) {
    ...
    DateTime reservedRangeStart = reservedRange.startDay!;
    return (reservedRangeStart.isAfter(selectedRangeStart) ||
            reservedRangeStart.isAtSameMomentAs(selectedRangeStart)) &&
        (reservedRangeStart.isBefore(selectedRangeEnd) ||
            reservedRangeStart.isAtSameMomentAs(
                selectedRangeEnd)); // is the reserved range start between selected range start and end?
  }
  ...
  String getDayPrice(DateTime day) {
    ...
    if (widget.rangeStartDay != null && widget.rangeEndDay != null) {
      PriceRange priceRange =
          PriceRange(startDay: widget.rangeStartDay, endDay: widget.rangeEndDay, price: widget.specialPrice);
      if ((day.isAfter(priceRange.startDay!) || day.isAtSameMomentAs(priceRange.startDay!)) &&
          (day.isBefore(priceRange.endDay!) || day.isAtSameMomentAs(priceRange.endDay!))) {
