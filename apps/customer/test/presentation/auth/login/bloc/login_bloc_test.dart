...
void main() {
  ...
  group('LoginUser event', () {
    blocTest('when logged in with correct credentials and authorized user emit [LoginLoading, LoginSuccess]',
        build: () {
          BaseResponse<LoginResponseEntity> authorizedUserResponse = BaseResponse(message: 'Authorized user', data: mockedUser);
          // when(() => mockedUser.isAuthorized).thenAnswer((invocation) => true);
          when(() => iAuthRepository.login(any(), any())).thenAnswer((_) async => Future.value(ApiResponse.success(authorizedUserResponse)));
          return loginBloc;
        },
        act: (LoginBloc bloc) => bloc.add(const LoginUser(true)),
    ...
    blocTest('when logged in with correct credentials but un authorized user emit [LoginLoading, LoginError]',
        build: () {
          BaseResponse<LoginResponseEntity> authorizedUserResponse = BaseResponse(message: 'un Authorized user', data: mockedUser);
    ...
    blocTest('when logged in with incorrect credentials emit [LoginLoading, LoginError]',
        build: () {
          when(() => iAuthRepository.login(any(), any()))
              .thenAnswer((_) async => Future.value(ApiResponse.failed(ApplicationError(errorMsg: '', type: Unauthorized()))));
          return loginBloc;
        },
        act: (LoginBloc bloc) => bloc.add(const LoginUser(true)),
        expect: () => [const LoginLoading()]);
