...
class GetOwnerService extends BaseService {
  Future<ApiResponse> getOwner(String id) async {
    NetworkRequest request = NetworkRequest(ownerApi + "/$id", RequestMethod.get, headers: await getHeaders());

    var result = await networkManager.perform(request);

    if (result.status == Status.OK) {
      result.data = BaseResponse.fromJson(result.data, (json) => Owner.fromJson(json));
    }
    return result;
  }
}
