...
class TableCalendar<T> extends StatefulWidget {
  ...
  final List<ActiveRange> pricesRanges;

  final List<ActiveReservation> reservedRanges;
...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  void _onDayTapped(DateTime day) {
    ...
    if (_isDayDisabled(day) && !isFirstDayInReservation(day)) {
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: Theme.of(context).secondaryHeaderColor, borderRadius: BorderRadius.circular(2)),
                  ),
                  const SizedBox(
                    width: 8,
                  ),
              ...
              Row(
                children: [
                  Container(
                    ...
                    height: 12,
                    decoration: BoxDecoration(
                        color: Theme.of(context).unselectedWidgetColor, borderRadius: BorderRadius.circular(2)),
                  ),
                  const SizedBox(
                    width: 8,
                  ),
              ...
              Row(
                children: [
                  Container(
                    ...
                    height: 12,
                    decoration:
                        BoxDecoration(color: reservedColor, borderRadius: BorderRadius.circular(2)),
                  ),
                  const SizedBox(
                    width: 8,
                  ),
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        Widget content = CellContent(
          ...
          isFirstDayInReservation: isFirstDayInReservation(day) && day.isAfter(kToday),
  ...
  bool _isToday(DateTime day) {
    return day.day == kToday.day && day.month == kToday.month && day.year == kToday.year;
  }
  ...
  // check if this day is first day in reservation
  // and no check out in same day
  bool isFirstDayInReservation(DateTime day) {
    bool isFirstDay = false;
    widget.reservedRanges.asMap().forEach((key, reservation) {
      if(!isFirstDay) {
        final firstReservationDay = reservation.startDay;
        isFirstDay = day.year == firstReservationDay?.year
            && day.month == firstReservationDay?.month
            && day.day == firstReservationDay?.day;
        if(isFirstDay) {
          final isSharedDay = widget.reservedRanges.firstWhereOrNull(
            (ActiveReservation res) =>
                res.endDay?.add(const Duration(days: 1)).compareTo(day) == 0,
          );
          if(isSharedDay != null) {
            isFirstDay = false;
          }
  ...
  bool isInRange(
    ActiveReservation reservedRange,
    DateTime start,
    DateTime end,
  ) {
    ...
    DateTime existingStartDate = reservedRange.startDay!;
    DateTime existingEndDate = reservedRange.endDay!;

    if (newEndDate.isBefore(existingStartDate) ||
        newStartDate.isAfter(existingEndDate)) {
      return false;
    }

    if (newEndDate.isAtSameMomentAs(existingStartDate)) {
      return false;
    }
  ...
  int getDayPrice(DateTime day) {
    int price = widget.defaultPrice;
